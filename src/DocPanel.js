...
export function createDocPanel() {
  ...
  if (savedPosition) {
    ...
    panel.style.width = savedPosition.width ? savedPosition.width + "px" : "600px";
    panel.style.height = savedPosition.height ? savedPosition.height + "px" : "400px";
  } else {
  ...
  rightContent.style.padding = "15px";
  ...
  const showFunctionDetails = (funcName) => {
    ...
    // Build the content
    let content = `
      <div class="function-header" style="border-bottom: 1px solid ${categoryColor}4D">
        <h2 style="color: ${categoryColor}">${funcName}()</h2>
        <div class="function-category" style="color: ${categoryColor}; font-size: 12px">
          ${category ? category.title : ""}
        </div>
      </div>
      <div class="function-description" style="margin: 15px 0">
    ...
    if (funcInfo.params && funcInfo.params.length > 0) {
      content += `
        <div class="function-params">
          <h3 style="font-size: 14px; margin: 15px 0 10px 0">Parameters</h3>
          <table style="width: 100%; border-collapse: collapse">
            <thead>
              <tr style="text-align: left; border-bottom: 1px solid rgba(var(--color-bg-tertiary-rgb), 0.5)">
      ...
      funcInfo.params.forEach(param => {
        content += `
          <tr style="border-bottom: 1px solid rgba(var(--color-bg-tertiary-rgb), 0.3)">
            <td style="padding: 5px"><code>${param.name}</code></td>
            <td style="padding: 5px"><code>${param.default || ''}</code></td>
            <td style="padding: 5px">${param.description || ''}</td>
          </tr>
        `;
      });
    ...
    if (funcInfo.example) {
      content += `
        <div class="function-example" style="margin: 15px 0">
          <h3 style="font-size: 14px; margin: 15px 0 10px 0">Example</h3>
          <pre style="background-color: rgba(0,0,0,0.2); padding: 10px; border-radius: 4px; overflow-x: auto; font-family: monospace">${funcInfo.example}</pre>
        </div>
      `;
    }
  ...
  Object.keys(FUNCTION_CATEGORIES).forEach(catKey => {
    ...
    categoryHeading.style.marginTop = "15px";
    categoryHeading.style.marginBottom = "8px";
    ...
    functionList.style.flexDirection = "column";
    functionList.style.marginLeft = "10px";
    
    category.functions.forEach(funcName => {
      const funcElement = document.createElement("div");
      funcElement.textContent = funcName;
      funcElement.style.padding = "3px 0";
      funcElement.style.cursor = "pointer";
      funcElement.style.transition = "all 0.1s";
      
      funcElement.addEventListener("mouseenter", () => {
        funcElement.style.color = category.color;
      });
      
      funcElement.addEventListener("mouseleave", () => {
        if (selectedFunction !== funcName) {
          funcElement.style.color = "var(--color-text-primary)";
        }
      });
      ...
      funcElement.addEventListener("click", () => {
        ...
        if (selectedFunction) {
          const elements = leftSidebar.querySelectorAll("div");
          elements.forEach(el => {
            if (el.textContent === selectedFunction) {
        ...
        funcElement.style.color = category.color;
        funcElement.style.fontWeight = "bold";
    ...
    leftSidebar.appendChild(functionList);
  });
  ...
  rightContent.innerHTML = `
    <div style="display: flex; flex-direction: column; justify-content: center; align-items: center; height: 100%; color: var(--color-text-secondary);">
      ...
      <div>Select a function from the left sidebar to view its documentation</div>
    </div>
  `;
